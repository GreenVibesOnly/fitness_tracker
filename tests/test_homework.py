...
try:
    ...
    assert False, 'Не найден файл с модулем `module.py`'
except NameError as exc:
    name = re.findall("name '(\w+)' is not defined", str(exc))[0]
    assert False, f'Класс {name} не обнаружен в файле модуля.'
except ImportError:
    assert False, 'Не найден файл с модулем `module.py`'
...
def test_read_package():
    assert hasattr(module, 'read_package'), (
        'Создайте функцию для обработки '
        'входящего пакета - `read_package`'
    )
    assert callable(module.read_package), (
        '`read_package` должна быть функцией.'
    )
    assert isinstance(module.read_package, types.FunctionType), (
        '`read_package` должна быть функцией.'
    )
...
def test_read_package_return(input_data, expected):
    result = module.read_package(*input_data)
...
def test_InfoMessage():
    assert inspect.isclass(module.InfoMessage), (
        '`InfoMessage` должен быть классом.'
    )
    info_message = module.InfoMessage
...
def test_InfoMessage_get_message(input_data, expected):
    info_message = module.InfoMessage(*input_data)
...
def test_Training():
    assert inspect.isclass(module.Training), (
    ...
    for name, value in {
        ...
        assert hasattr(module.Training, name), (
            f'У класса `Training` должна быть константа `{name}`'
            f'{value.description}.'
        )
        assert getattr(module.Training, name) == value.value, (
            f'Значение константы `{name}` в классе `Training` должно быть '
            f'равно `{value.value}`.'
        )
    MIN_IN_H = 60
    assert MIN_IN_H in module.Training.__dict__.values(), (
...
def test_Training_get_distance(input_data, expected):
    training = module.Training(*input_data)
...
def test_Training_show_training_info(monkeypatch):
    training = module.Training(*[720, 1, 80])
...
def test_Swimming():
    assert hasattr(module, 'Swimming'), 'Создайте класс `Swimming`'
    assert inspect.isclass(module.Swimming), (
        '`Swimming` должен быть классом.'
    )
    assert issubclass(module.Swimming, module.Training), (
        'Класс `Swimming` должен наследоваться от класса `Training`.'
    )
    Swimming_attr_values = module.Swimming.__dict__.values()
    for value_info in {
...
def test_Swimming_get_mean(input_data, expected):
    swimming = module.Swimming(*input_data)
...
def test_SportsWalking():
    assert hasattr(module, 'SportsWalking'), 'Создайте класс `SportsWalking`'
    assert inspect.isclass(module.SportsWalking), (
        '`SportsWalking` должен быть классом.'
    )
    assert issubclass(module.SportsWalking, module.Training), (
        'Класс `SportsWalking` должен наследоваться от класса `Training`.'
    )
    SportsWalking_attr_values = module.SportsWalking.__dict__.values()
    for value_info in {
            'CALORIES_WEIGHT_MULTIPLIER': ValueInfo(
                0.035,
    ...
    sports_walking = module.SportsWalking
...
def test_SportsWalking_get_spent_calories(input_data, expected):
    sports_walking = module.SportsWalking(*input_data)
...
def test_Running():
    assert hasattr(module, 'Running'), 'Создайте класс `Running`'
    assert inspect.isclass(module.Running), '`Running` должен быть классом.'
    assert issubclass(module.Running, module.Training), (
        'Класс `Running` должен наследоваться от класса `Training`.'
    )
    Running_attr_values = module.Running.__dict__.values()
    for value_info in {
...
def test_Running_get_spent_calories(input_data, expected):
    running = module.Running(*input_data)
...
def test_main():
    ...
    assert callable(module.main), '`main` должна быть функцией.'
...
def test_main_output(input_data, expected):
    with Capturing() as get_message_output:
        training = module.read_package(*input_data)
